(function($) {
  ...
  $.fn.mDropdown = function(options) {
    ...
    var Plugin = {
      ...
      init: function(options) {
        ...
        dropdown.defaultDropPos = element.hasClass("m-dropdown--up")
          ? "up"
          : "down";
      ...
      build: function() {
        if (mUtil.isMobileDevice()) {
          if (
            element.data("dropdown-toggle") == "hover" ||
            element.data("dropdown-toggle") == "click"
          ) {
      ...
      adjustArrowPos: function() {
        ...
        var alignment = dropdown.arrow.hasClass("m-dropdown__arrow--right")
          ? "right"
          : "left";
        var pos = 0;
        ...
        if (dropdown.arrow.length > 0) {
          if (
            ...
            element.hasClass("m-dropdown--mobile-full-width")
          ) {
            pos =
              element.offset().left +
              width / 2 -
              Math.abs(dropdown.arrow.width() / 2) -
              parseInt(dropdown.wrapper.css("left"));
  ...
  $(document).on("click", function(e) {
    $(".m-dropdown.m-dropdown--open").each(function() {
      ...
      if (
        toggle.length > 0 &&
        target.is(toggle) !== true &&
        toggle.find(target).length === 0 &&
        target.find(toggle).length === 0 &&
        dropdown.isPersistent() == false
      ) {
        dropdown.hide();
